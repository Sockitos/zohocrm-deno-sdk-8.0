  ...
  public async add<T>(header: Header<T>, value: T) {
    ...
    try {
      ...
      if (headerClassName !== undefined && headerClassName != null) {
        parsedHeaderValue = await new HeaderParamValidator()
          .validate(headerName, headerClassName, value)
          .catch((err) => {
            throw err;
          });
      } else {
      ...
      if (
        this.headerMap.has(headerName) &&
        this.headerMap.get(headerName) != null
      ) {
...
export { HeaderMap as HeaderMap, HeaderMap as MasterModel };
