...
import { HeaderMap } from "../header_map.ts";
import { Initializer } from "../initializer.ts";
import { Param } from "../param.ts";
import { ParameterMap } from "../parameter_map.ts";
// Path operations replaced with URL-based approach
// OS module operations replaced with Deno equivalents
...
import { APIResponse } from "../controllers/api_response.ts";
// IncomingHttpHeaders type replaced with Headers from Deno's fetch API"
...
class CommonAPIHandler {
  ...
  public setHeader(header: HeaderMap | undefined | null) {
    ...
    if (
      this.headers.getHeaderMap() != null &&
      this.headers.getHeaderMap().size > 0
    ) {
  ...
  public async apiCall<T>(
    ...
    if (
      ...
      try {
        ...
        if (converterInstance !== null) {
          var classFileName = await converterInstance
            .getFileName(this.request.constructor.name)
            .catch((err) => {
              throw err;
            });
    ...
    try {
      ...
      let headerMap = await this.getHeaders(response.headers).catch((err) => {
        throw err;
      });
      ...
      if (
        ...
        let contentTypeHeader: string | string[] | undefined =
  ...
  private async getHeaders(headers: Headers): Promise<Map<string, string>> {
    let headerMap: Map<string, string | undefined | string[]> = new Map<
      string,
