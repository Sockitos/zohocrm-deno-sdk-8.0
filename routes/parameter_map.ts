  ...
  public async add<T>(param: Param<T>, value: T) {
    if (param == null || param == undefined) {
      throw new SDKException(
        Constants.PARAMETER_NULL_ERROR,
        Constants.PARAM_INSTANCE_NULL_ERROR
      );
    }
    let paramName = param.getName();
    if (paramName == null || paramName == undefined) {
      throw new SDKException(
        Constants.PARAM_NAME_NULL_ERROR,
        Constants.PARAM_NAME_NULL_ERROR_MESSAGE
      );
    }
    if (value == null) {
      throw new SDKException(
    ...
    try {
      ...
      if (paramClassName !== undefined && paramClassName !== null) {
        parsedParamValue = await new HeaderParamValidator()
          .validate(paramName, paramClassName, value)
          .catch((err) => {
            throw err;
          });
      } else {
      ...
      if (
        this.parameterMap.has(paramName) &&
        this.parameterMap.get(paramName) != null
      ) {
...
export { ParameterMap as MasterModel, ParameterMap as ParameterMap };
