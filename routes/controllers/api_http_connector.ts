...
export class APIHTTPConnector {
  ...
  public async fireRequest(converterInstance: Converter | null) {
    ...
    if (
      ...
      Array.from(Object.keys(this.requestBody)).length > 0 &&
      converterInstance !== null
    ) {
      modifiedRequestBody = await converterInstance
        .appendToRequest(this)
        .catch((err) => {
          throw err;
        });
    }
    ...
    let timeout: number =
      initializer.getSDKConfig() != null
        ? initializer.getSDKConfig().getTimeout()
        : 0;
    if (timeout > 0) {
    ...
    if (requestProxy !== undefined) {
      let proxyAuthorization: string | undefined = undefined;
