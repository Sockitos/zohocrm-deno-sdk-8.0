...
// Replaced 'got' with native fetch API
// Using native FormData API instead of form-data package
...
export class OAuthToken implements Token {
  ...
  public async getToken() {
    let initializer: Initializer = await Initializer.getInitializer().catch(
      (err) => {
        throw err;
      }
    );
    let store = initializer.getStore();
    let oauthToken: Token | null | void = null;
    let idToSearch = this.getId();
    if (idToSearch != null) {
      oauthToken = await store.findTokenById(idToSearch).catch((err) => {
        throw err;
      });
      ...
      oauthToken = await store.findToken(this).catch((err) => {
        throw err;
      });
    }
    if (oauthToken == null) {
    ...
    if (oauthToken instanceof OAuthToken) {
      if (
        oauthToken.getAPIDomain() == null ||
        oauthToken.getAPIDomain()!.length <= 0
      ) {
        if (initializer.getEnvironment() == null) {
          throw new SDKException(
            Constants.ENVIRONMENT_ERROR_1,
            Constants.ENVIRONMENT_ERROR_MESSAGE.concat(" : ")
          );
        }
      ...
      if (api_domain != null) {
        ...
        if (environment != null) {
          try {
            let initializer = await Initializer.getInitializer().catch(
              (err) => {
                throw err;
              }
            );
            ...
            if (error instanceof SDKException) {
              ...
              throw new SDKException(
                Constants.ENVIRONMENT_ERROR_RESPONSE.concat(" : "),
                null,
                null,
                error
              );
      ...
      if (
        ...
        oauthToken.getAccessToken()!.length == 0
      ) {
        if (
        ...
        try {
          if (oauthToken.getUserSignature() == null && oauthToken.findUser) {
            try {
              ...
              let userName: string | null = await utility
                .getUserName(oauthToken.accessToken)
                .catch((err) => {
                  throw err;
                });
              if (userName != null) {
  ...
  private async checkTokenDetails() {
    if (this.getGrantToken() == null && this.getRefreshToken() == null) {
      throw new SDKException(
        ...
        Constants.GET_TOKEN_BY_USER_NAME_ERROR +
          " - " +
          Constants.OAUTH_MANDATORY_KEYS2.join()
      );
    }
    return true;
  }
  ...
  private async mergeObjects(
    first: OAuthToken | null,
    second: Token | void | null
  ) {
  ...
  private async refreshAccessToken(oauthToken: OAuthToken, url: string) {
    ...
    var response = await this.getResponse(url, requestDetails).catch((err) => {
      throw err;
    });
    try {
  ...
  async getResponse(url: string, requestDetails: RequestInit) {
  ...
  async parseResponse(oauthToken: OAuthToken, response: string) {
    ...
    if (
      sdkConfig.applyAPIDomainUpdate() &&
      responseJSON.hasOwnProperty(Constants.API_DOMAIN)
    ) {
      oauthToken.setAPIDomain(responseJSON[Constants.API_DOMAIN]);
    }
    return oauthToken;
  ...
  public async revoke(id: string): Promise<boolean | undefined> {
    try {
      if (
        (await Initializer.getInitializer().catch((err) => {
          throw err;
        })) == null
      ) {
        throw new SDKException(
      ...
      if (oauthToken != null && oauthToken.getRefreshToken() != null) {
        isRevoke = await this.revokeRefreshToken(
          oauthToken.getRefreshToken(),
          url + Constants.REVOKE_URL
        ).catch((err) => {
          throw err;
        });
  ...
  public async revokeRefreshToken(
    ...
    try {
      let response = await this.getResponse(url, requestDetails).catch(
        (err) => {
          throw err;
        }
      );
      ...
      return !!(
        responseJSON != null &&
        responseJSON.has(Constants.STATUS) &&
        responseJSON[Constants.STATUS] == Constants.STATUS_SUCCESS
      );
    } catch (error) {
      if (error instanceof SDKException) {
        throw error;
