...
export class FileStore implements TokenStore {
  ...
  async findToken(token: OAuthToken): Promise<Token | null> {
    try {
      ...
      throw new SDKException(
        ...
        Constants.GET_TOKEN_FILE_ERROR,
        null,
        error
      );
    }
  ...
  async saveToken(token: OAuthToken): Promise<void> {
    try {
        ...
        if (nextRecord.length > 1) {
          ...
          if (isRowPresent) {
            ...
            csvReader.splice(
              index,
              1,
              (await this.setToken(oauthToken)).join(",")
            );
            break;
          }
        } else {
      ...
      if (!isRowPresent) {
        if (
          oauthToken.getId() != null ||
          oauthToken.getUserSignature() != null
        ) {
          if (
            oauthToken.getRefreshToken() == null &&
            oauthToken.getGrantToken() == null &&
            oauthToken.getAccessToken() == null
          ) {
            throw new SDKException(
              Constants.TOKEN_STORE,
              Constants.GET_TOKEN_FILE_ERROR1
            );
          }
        }
        if (oauthToken.getId() == null) {
      ...
      if (err instanceof SDKException) {
        ...
        throw new SDKException(
          Constants.TOKEN_STORE,
          Constants.SAVE_TOKEN_FILE_ERROR,
          null,
          err
        );
      }
  ...
  async deleteToken(id: string): Promise<void> {
    try {
      ...
      if (isRowPresent) {
        ...
        throw new SDKException(
          Constants.TOKEN_STORE,
          Constants.GET_TOKEN_BY_ID_FILE_ERROR
        );
      }
    } catch (err) {
      if (err instanceof SDKException) {
        ...
        throw new SDKException(
          Constants.TOKEN_STORE,
          Constants.DELETE_TOKEN_FILE_ERROR,
          null,
          err
        );
      }
  ...
  async getTokens(): Promise<Token[] | null> {
    try {
      ...
      throw new SDKException(
        Constants.TOKEN_STORE,
        Constants.GET_TOKENS_FILE_ERROR,
        null,
        error
      );
    }
  ...
  async deleteTokens(): Promise<void> {
    try {
      ...
      throw new SDKException(
        Constants.TOKEN_STORE,
        Constants.DELETE_TOKENS_FILE_ERROR,
        null,
        error
      );
    }
  ...
  async checkCondition(
    ...
    if (oauthToken.getUserSignature() != null) {
      ...
      if (
        ...
        grantToken.length > 0 &&
        oauthToken.getGrantToken() != null &&
        oauthToken.getGrantToken() == grantToken
      ) {
        isRowPresent = true;
      } else if (
        refreshToken != null &&
        ...
        oauthToken.getRefreshToken() != null &&
        oauthToken.getRefreshToken() == refreshToken
      ) {
        isRowPresent = true;
      }
    }
  ...
  async getData(value: string | null) {
    return value != null && value.length > 0 && !(value.length == 0)
      ? value
      : null;
  }
  async setToken(oauthToken: OAuthToken) {
    ...
    data[1] =
      oauthToken.getUserSignature() != null
        ? oauthToken.getUserSignature()?.getName()
        : null;
