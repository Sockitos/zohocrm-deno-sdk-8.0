...
export class JSONConverter extends Converter {
  ...
  public async appendToRequest(requestBase: object): Promise<any> {
    ...
    return JSON.stringify(request.getRequestBody(), (key, value) =>
      typeof value === Constants.BIGINT_TYPE ? value.toString() : value
    );
  }

  public async formRequest(
    requestInstance: object,
    pack: string,
    instanceNumber: number | null,
    memberDetail: object | null
  ): Promise<object> {
    var classDetail = Initializer.jsonDetails[pack];
    if (
      classDetail.hasOwnProperty(Constants.INTERFACE) &&
      classDetail[Constants.INTERFACE] === true
    ) {
      var classes = classDetail[Constants.CLASSES];
      var baseName = pack.split("/").slice(0, -1);
      let className = await this.getFileName(
  ...
  private async isNotRecordRequest(
    requestInstance: any,
    classDetail: { [key: string]: any },
    instanceNumber: number | null,
    classMemberDetail: { [key: string]: any } | null
  ): Promise<object> {
    ...
    if (classMemberDetail !== null) {
      ...
      skipMandatory = classMemberDetail.hasOwnProperty(Constants.SKIP_MANDATORY)
        ...
        : false;
      classMemberName = await this.buildName(
        classMemberDetail[Constants.NAME]
      ).catch((err) => {
        throw err;
      });
    }
    ...
    for (let memberName in classDetail) {
      ...
      if (
        ...
        if (
          keyName.toLowerCase() === Constants.BODY.toLowerCase() &&
          this.commonAPIHandler
            .getAPIPath()
            .endsWith(Constants.FUNCTIONS_PATH) &&
  ...
  private checkException(
    ...
    instanceNumber: number | null,
    lookUp: boolean,
    requiredKeys: Map<string, boolean>,
    primaryKeys: Map<string, boolean>,
    requiredInUpdateKeys: Map<string, boolean>
  ): boolean {
    if (
  ...
  // isRecordRequest
  private async isRecordRequest(
    recordInstance: any,
    classDetail: { [key: string]: any },
    instanceNumber: number | null,
    memberDetail: { [key: string]: any } | null
  ): Promise<any> {
    ...
    for (let keyName of Array.from(keyModified.keys())) {
      ...
      let keyValue: any = keyValues.has(keyName)
      ...
      let memberName: string = await this.buildName(keyName).catch((err) => {
        throw err;
      });
      ...
      if (keyValue != null) {
        if (keyDetail != null && Object.keys(keyDetail).length > 0) {
          ...
          if (
            ...
            !(keyValue instanceof Map) &&
            !(keyValue instanceof Choice)
          ) {
            let key = await this.keyINJSONDetails(
              keyValue.constructor.name
            ).catch((err) => {
              throw err;
            });
            if (keyValue instanceof Date) {
              if (
                ...
                jsonValue = await DatatypeConverter.postConvert(
                  keyValue,
                  "date"
                );
              } else {
            ...
            if (Constants.PRIMITIVE_TYPES.includes(typeof keyValue)) {
              ...
              jsonValue = await this.setData(keyDetail, keyValue).catch(
                (err) => {
                  throw err;
                }
              );
            ...
            if (
              customHandling &&
              keyValue.constructor.name == Constants.ARRAY_KEY &&
              keyValue.length > 0
            ) {
              let key1 = await this.keyINJSONDetails(
                keyValue[0].constructor.name
              ).catch((err) => {
                throw err;
              });
              if (key1 != null) {
                ...
                jsonValue = await this.redirectorForObjectToJSON(
                  keyValue
                ).catch((err) => {
                  throw err;
                });
  ...
  private async setData(
    memberDetail: { [key: string]: any },
    fieldValue: any
  ): Promise<any> {
    if (fieldValue != null) {
      let type = memberDetail[Constants.TYPE].toString();
      return await this.setDataValue(type, memberDetail, fieldValue).catch(
        (err) => {
          throw err;
        }
      );
  ...
  private async setDataValue(
    ...
    if (type.toLowerCase() == Constants.LIST_NAMESPACE.toLowerCase()) {
      return await this.setJSONArray(fieldValue, memberDetail).catch((err) => {
        throw err;
      });
      ...
      return await DatatypeConverter.postConvert(
        fieldValue,
        type.toLowerCase()
      );
    }
  ...
  private async setJSONArray(
    ...
    if (requestObjects.length > 0) {
      if (
        ...
        if (
          memberDetail != null &&
          memberDetail.hasOwnProperty(Constants.SUB_TYPE)
        ) {
          ...
          if (type == Constants.CHOICE_NAMESPACE) {
            ...
            for (let request of requestObjects) {
              jsonArray.push(
                await this.setDataValue(type, memberDetail, request)
              );
            }
          }
        } else {
  ...
  public async getWrappedResponse(
    response: any,
    pack: string
    ...
    if (response.body.length != 0) {
      ...
      return [
        await this.getResponse(responseJson, pack).catch((err) => {
          throw err;
        }),
        responseJson,
      ];
  ...
  public async getResponse(
    ...
    var classDetail: { [key: string]: any } =
      Initializer.jsonDetails[packageName];
    ...
    if (
      ...
      if (instance instanceof Record) {
        ...
        let moduleAPIName: string | null =
          this.commonAPIHandler.getModuleAPIName();
  ...
  private async notRecordResponse(
    instance: any,
    responseJSON: any,
    classDetail: { [key: string]: any }
  ): Promise<any> {
    for (let memberName in classDetail) {
      ...
      let keyName: string = keyDetail.hasOwnProperty(Constants.NAME)
  ...
  private async isRecordResponse(
    responseJson: any,
    classDetail: { [key: string]: any },
    pack: string
  ): Promise<any> {
    ...
    var recordDetail: { [key: string]: any } =
      Initializer.jsonDetails[Constants.RECORD_NAMESPACE];
  ...
  private async getData(
    ...
    if (keyData !== null) {
      let type = memberDetail[Constants.TYPE].toString();
      memberValue = await this.getDataValue(type, keyData, memberDetail).catch(
        (err) => {
          throw err;
        }
      );
  ...
  async getDataValue(
    ...
    let memberValue: any = null;
    if (type.toLowerCase() == Constants.LIST_NAMESPACE.toLowerCase()) {
      memberValue = await this.getCollectionsData(keyData, memberDetail).catch(
        (err) => {
          throw err;
        }
      );
      ...
      memberValue = await DatatypeConverter.preConvert(
        keyData,
        type.toLowerCase()
      );
    }
  ...
  private async getCollectionsData(
    responses: any[],
    memberDetail: { [key: string]: any } | null
  ): Promise<any[]> {
      ...
      if (
        ...
        if (
          ...
          if (type == Constants.CHOICE_NAMESPACE) {
            ...
            for (let response of responses) {
              values.push(
                await this.getDataValue(type, response, memberDetail).catch(
                  (err) => {
                    throw err;
                  }
                )
  ...
  private async findMatch(classes: string[], responseJson: any) {
    ...
    for (let className of classes) {
      var matchRatio: number = await this.findRatio(
        className,
        responseJson
      ).catch((err) => {
        throw err;
    ...
    return this.getResponse(responseJson, pack).catch((err) => {
      throw err;
    });
  }
  ...
  private findRatio(className: string, responseJson: any): Promise<number> {
    var classDetail: { [key: string]: any } =
      Initializer.jsonDetails[className];
