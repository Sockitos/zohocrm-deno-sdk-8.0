...
class DatatypeConverter {
  ...
  private static init() {
    ...
    var stringtoTimeZone = (obj: any) => {
      return moment.tz.zone(obj);
    };
    var timeZoneToString = (obj: any) => {
      return obj.name;
    };
    ...
    this.addToMap(
      Constants.DATE_NAMESPACE.toLowerCase(),
      stringToDate,
      dateToString
    );
    this.addToMap(
      Constants.DATETIME_NAMESPACE.toLowerCase(),
      stringToDateTime,
      dateTimeToString
    );
    ...
    this.addToMap(
      Constants.OBJECT_NAMESPACE.toLowerCase(),
      preObject,
      postObject
    );
    this.addToMap(
      Constants.TIMEZONE_NAMESPACE.toLowerCase(),
      stringtoTimeZone,
      timeZoneToString
    );
    ...
    this.addToMap(
      Constants.DATE_TYPE.toLowerCase(),
      stringToDateTime,
      dateTimeToString
    );
    this.addToMap(Constants.NUMBER_TYPE.toLowerCase(), double, double);
    this.addToMap(
      Constants.LOCALTIME_NAMESPACE.toLowerCase(),
      stringtoLocalTime,
      localTimetoSting
    );
  }
  ...
  private static async postConvertObjectData(obj: any) {
    if (Array.isArray(obj) && obj.length > 0) {
      ...
      for (let data of obj) {
        if (data instanceof Date) {
          ...
          await this.postConvertObjectData(data);
        } else {
          list.push(data);
        }
      }
      ...
      for (let key of Array.from(obj.keys())) {
        ...
        if (Array.isArray(value)) {
          requestObject[key] = await this.postConvertObjectData(value);
          ...
          if (
            ...
            requestObject[key] = await this.postConvert(
              value,
              Constants.DATE_NAMESPACE.toLowerCase()
            );
          } else {
