...
export abstract class Converter {
   ...
   * @throws {Error}
   */
  public abstract getWrappedResponse(
    response: any,
    pack: string
   ...
   * @throws {SDKException}
   */
  public async valueChecker(
    className: string,
    memberName: string,
    keyDetails: { [key: string]: any },
    value: any,
    uniqueValuesMap: Map<string, any[]>,
    instanceNumber: number | null
  ): Promise<boolean> {
    ...
    let givenType: string | null = null;
    if (keyDetails[Constants.INTERFACE]) {
      let interfaceDetail =
        Initializer.jsonDetails[keyDetails[Constants.STRUCTURE_NAME]];
    ...
    if (Constants.TYPE_VS_DATATYPE.has(type.toLowerCase())) {
      if (
        ...
        for (let data of value) {
          if (!(data instanceof className)) {
            ...
            for (
              var nameIndex = 0;
              nameIndex < classNameSplit.length;
              nameIndex++
            ) {
              ...
              expectedClassName =
                expectedClassName.concat(firstLetterUppercase);
            }
        ...
        Array.isArray(value) &&
        keyDetails.hasOwnProperty(Constants.SUB_TYPE)
      ) {
        let index = 0;
        for (let data of value) {
          ...
          if (sub_type.toLowerCase() == Constants.OBJECT_KEY) {
            ...
            dataType = Constants.SPECIAL_TYPES.has(sub_type.toLowerCase())
              ? Constants.SPECIAL_TYPES.get(sub_type.toLowerCase())
              : sub_type;
            if (Constants.TYPE_VS_DATATYPE.has(dataType.toLowerCase())) {
              if (
                Object.prototype.toString.call(data) !=
                Constants.TYPE_VS_DATATYPE.get(sub_type.toLowerCase())
              ) {
                check = false;
              }
            }
        ...
        //TypeScript don't have int type
        check =
      ...
      if (expectedStructure == "TimeZone") {
        ...
        if (!keyDetails[Constants.INTERFACE]) {
          let className = (await import("../../" + expectedStructure))
            .MasterModel;
          if (!(value instanceof className)) {
            check = false;
          ...
          if (check == false) {
            if (
              givenType?.toLowerCase() == Constants.INVENTORYTEMPLATE &&
              type.toLowerCase() == Constants.TEMPLATE
            ) {
              check = true;
            }
          }
    ...
    if (!check) {
      ...
      detailsJO[Constants.ACCEPTED_TYPE] = Constants.SPECIAL_TYPES.has(type)
    ...
    if (
      ...
      (!keyDetails.hasOwnProperty(Constants.PICKLIST) ||
        (keyDetails[Constants.PICKLIST] &&
          initializer.getSDKConfig().getPickListValidation()))
    ) {
      let valuesJA = keyDetails[Constants.VALUES];
      await Converter.checkChoiceValue(
        memberName,
        className,
        instanceNumber,
        value,
    ...
    if (
      ...
      if (
        ...
        detailsJO[Constants.ERROR_HASH_MAXIMUM_LENGTH] =
          keyDetails[Constants.MAX_LENGTH];
      ...
      if (
        ...
        detailsJO[Constants.ERROR_HASH_CLASS] = className;
        detailsJO[Constants.GIVEN_LENGTH] = count;
        detailsJO[Constants.ERROR_HASH_MINIMUM_LENGTH] =
          keyDetails[Constants.MIN_LENGTH];
  ...
  public static async checkChoiceValue1(
    className: string,
    keyDetails: { [key: string]: any },
    value: object
  ) {
    ...
    if (
      ...
      await this.checkChoiceValue(
        name,
        className,
        0,
        value,
        {},
        keyDetails[Constants.VALUES]
      );
    }
  }

  public static async checkChoiceValue(
    memberName: string,
    className: string,
    instanceNumber: number | null,
    value: object,
    detailsJO: { [key: string]: any },
    valuesJA: any
  ) {
    if (Array.isArray(value)) {
      for (let value_2 of value) {
        ...
        if (!valuesJA.includes(value_2)) {
          ...
          throw new SDKException(
            Constants.UNACCEPTED_VALUES_ERROR,
            null,
            detailsJO
          );
        }
