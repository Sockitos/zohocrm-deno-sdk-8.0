...
import { JSONConverter } from "./json_converter.ts";
...
class HeaderParamValidator {
  public async validate(name: string, className: string, value: any) {
    let jsonDetails: { [key: string]: any } = await this.getJSONDetails().catch(
      (err) => {
        throw err;
      }
    );
    ...
    if (typeDetail !== null) {
      if (typeDetail.hasOwnProperty(Constants.STRUCTURE_NAME)) {
          ...
          for (let response of value) {
            jsonArray.push(
              await converter
                .formRequest(
                  response,
                  typeDetail[Constants.STRUCTURE_NAME],
                  0,
                  null
                )
                .catch((err) => {
        ...
        return JSON.stringify(
          await converter
            .formRequest(value, typeDetail[Constants.STRUCTURE_NAME], 0, null)
            .catch((err) => {
              throw err;
            })
        );
      } else if (
        typeDetail.hasOwnProperty(Constants.TYPE) &&
        typeDetail[Constants.TYPE].toLowerCase() ==
          Constants.CHOICE_NAMESPACE.toLowerCase()
      ) {
        await Converter.checkChoiceValue1(
      ...
      return (
        await this.parseData(value).catch((err) => {
          throw err;
        })
      ).toString();
    }
    ...
    if (Array.isArray(value)) {
      ...
      if (value.length > 0) {
        for (let request of value) {
          jsonArray.push(
            await this.parseData(request).catch((err) => {
              throw err;
            })
          );
        }
      ...
      for (let key of Array.from(value.keys())) {
        jsonObject[key] = await this.parseData(value.get(key)).catch((err) => {
          throw err;
        });
      }
      return jsonObject;
      ...
      let type = Object.prototype.toString.call(value);
      return (
        await DatatypeConverter.postConvert(value, type.toLowerCase())
      ).toString();
    }
  }
  ...
  private async getSplitFileName(className: string) {
    let fileName = <string[]>[];
    let nameParts = className.split(/([A-Z][a-z]+)/).filter(function (e) {
      return e;
    });
  ...
  private async getKeyJSONDetails(
    name: string,
