...
import { StreamWrapper } from "./stream_wrapper.ts";
...
class FormDataConverter extends Converter {
  ...
  public async appendToRequest(requestBase: object): Promise<any> {
    ...
    await this.addFileBody(request.getRequestBody(), formDataRequestBody).catch(
      (err) => {
        throw err;
      }
    );
  ...
  public async getWrappedResponse(
    ...
    return [await this.getResponse(responseObject, pack)];
  }
